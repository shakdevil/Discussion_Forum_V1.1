import { db } from './db';
import { questions, answers, type InsertQuestion, type InsertAnswer } from '@shared/schema';
...
async function isDataLoaded(): Promise<boolean> {
  ...
  return questionCount.length > 0;
}

// Function to add sample data to the database
export async function addDbSampleData() {
  ...
  // Insert all answers
  await db.insert(answers).values(allAnswers);

  console.log('✅ Sample data has been added to the PostgreSQL database!');
}